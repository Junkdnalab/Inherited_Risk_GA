...
p1 <- ggplot(fitness.res, aes(x = gen, y = avg, colour = type)) +
  ...
  scale_color_manual(values = kelly.colours[3:5]) +
  theme_bw() +
...
library(ggridges)
p4 <- ggplot(lastgen, aes(x = fitness, y = type, fill = type)) + geom_density_ridges() +
  xlab("Fitness Score") + ylab("") + guides(fill = FALSE) +
  scale_fill_manual(values = kelly.colours[3:5]) +
...
for(n in rownames(b_data)) {
  ...
  res <- data.frame(obj.fun = n,
                    breast.hits = breast[1],
                    breast.nohits  = breast[2],
                    ctrl.hits = ctrl[1],
                    ctrl.nohits = ctrl[2],
