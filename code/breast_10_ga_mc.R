...
source("/drive-pool/data/peter_data/repo/Inherited_Risk_GA/code/gp_fun_update.R")
...
## Gene expression by cell type info
load("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/gwas_gp_manu_data/breast_ct_exp.rda")
ct.exp.lookup <- ct_exp %>% dplyr::filter(expressed == TRUE) %>%
  dplyr::select(gene_name, type) %>% distinct() %>% remove_rownames() %>%
  mutate(tf.exp = 1)

## Load motifbreakr result and filter for strong effect
# load("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/gwas_gp_manu_data/motifbreakr_brca.rda"); rm(brca.mb.res.rsid)
# strong.mb.disrupt <- get.mb.strong(data = motifbreakr.results)
load("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/gwas_gp_manu_data/tfbs.lookup.rda") ## filter by expression already

## Load htftarget data
htftarget <- fread("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/gwas_gp_manu_data/hTFtarget.txt")
breast.htftarget <- htftarget %>% .[grep(pattern = "breast", x = tissue),] %>% 
  dplyr::mutate(htftarget = 1) %>% 
  dplyr::select(-tissue) %>%
  distinct()

## Load motifbreakr remap objective function data
load("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/gwas_gp_manu_data/mb.remap.df.rda")
## Prep lookup table to add cell type info
mb.remap.ct <- mb.remap.df %>% left_join(x = ., y = ct.exp.lookup, by = c("gene"="gene_name"), relationship = "many-to-many") %>%
  na.omit() %>% distinct() %>% ## Remove NA and keep distinct observations
  mutate(mb.remap = 1) ## Add mb.remap column
...
obj.fun <- c("marker.gene", "magma", "cancer.gene", "ppi", "marker.ppi", "inter.ppi", "intra.ppi",
             "lncrna", "promoter","marker.atac", "common.atac", "htftarget", "mb.remap")

n.run <- data.frame(run = 1:3) %>% t() %>% as.data.frame() %>% as.list() ## HOw many runs are we doing? 10
...
mclapply(X = n.run, FUN = function(x) {
  ...
  save(multi.gp, file = paste0("/drive-pool/data/peter_data/repo/Inherited_Risk_GA/result/progressive_breast_run_", num.run , "_ppithres", ppi.threshold, "_110424.rda"))
}, mc.cores = 10)
