...
ga.run <- gwas_gp(init.proposal = multi_proposal, generations = 10, obj.fun = obj.fun, khan.method = TRUE)
...
date()
save(multi.gp, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/multi.run_110723.rda")
...
## Result is from using GA last gen proposals top calls for each 
## Data must have locus, gene and celltype information
data <- result[, c("locus", "gene", "celltype")] 

get.hits <- function(data) { 
  if("marker.gene" %in% obj.fun) {
    ## Prepare marker df
    of1 <- combined.gene.marker %>% mutate(hit = 1) %>% dplyr::select(-celltype) %>% distinct()
    d1 <- data %>% left_join(x = ., y = of1, by = "gene") %>%
      mutate(hit = replace_na(hit, 0)) %>%
      dplyr::group_by(locus) %>%
      dplyr::summarise(marker.gene = sum(hit)) %>%
      mutate(marker.gene = case_when(marker.gene > 0 ~ 1,
                                     marker.gene == 0 ~ 0))
  }
  if("magma" %in% obj.fun) {
    d2 <- data %>% left_join(x = ., y = magma, by = "gene") %>%
  ...
  if("hi.mut" %in% obj.fun) {
    d3 <- data %>% left_join(x = ., y = breast.cancer.gene, by = "gene") %>%
