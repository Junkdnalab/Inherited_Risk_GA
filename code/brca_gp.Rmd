...
gwas.catalog <- fread(file = "/drive-pool/data/peter_data/genetic_programming/gwas/gwas_catalog_v1.0.2-associations_e109_r2023-06-03.tsv")

## Filter for phenotype of interest. Not sure whats available, but will be selecting phenotypes with breast or brca in it.
toi <- c("breast", "brca")

b.phenotype <- unique(gwas.catalog$MAPPED_TRAIT) %>%
  .[grep(pattern = paste(toi,collapse="|"), x = .)]
## Looking through b.phenotype, breast carcinoma and breast cancer look like the best candidates.

## Filter for those two traits
b.gwas <- gwas.catalog %>% dplyr::filter(MAPPED_TRAIT %in% c("breast carcinoma", "breast cancer"))
rm(toi, b.phenotype, gwas.catalog); gc()

## Lets look at what study to choose
b.studies <- b.gwas[, c("DATE ADDED TO CATALOG", "PUBMEDID", "FIRST AUTHOR", "DATE", "JOURNAL", "LINK", "STUDY", "DISEASE/TRAIT", "MAPPED_TRAIT")] %>%
  distinct()

## We are going to chose PMID 32424353 and 29059683. PMID 32424353 is 133,384 breast cancer cases and 113,789 controls identified 32 more novel snps in additional to previous identified snps (PMID 29059683).
signif.pval <- 5e-8

gwas_32424353_29059683 <- b.gwas %>% dplyr::filter(PUBMEDID %in% c(32424353, 29059683)) %>% 
  dplyr::rename("PVALUE" = "P-VALUE") %>%
  mutate(PVALUE = as.numeric(PVALUE)) %>%
  dplyr::filter(PVALUE <= signif.pval) %>%
  dplyr::select(REGION, CHR_ID, CHR_POS, SNPS)

## If snp label has chr in it. We need to process it so we can extract the chr_id and pos and then determine region.
if(length(grep(pattern = ":", x = gwas_32424353_29059683$SNPS)) > 0) {
  nonrsid.cases <- grep(pattern = ":", x = gwas_32424353_29059683$SNPS)
  for(n in nonrsid.cases) {
    df <- gwas_32424353_29059683[n, ]$SNPS %>% strsplit(x = ., split = ":") %>% unlist()
    if(grepl(pattern = "chr", x = df[1])) {
      chr_id <- substring(text = df[1], first = 4, last = 5)
    } else {
      chr_id <- df[1]
    }
    gwas_32424353_29059683[n, "CHR_ID"] <- chr_id
    gwas_32424353_29059683[n, "CHR_POS"] <- as.numeric(df[2])
    gwas_32424353_29059683[n, "REGION"] <- position2Cytoband(chrom = chr_id, 
                                                             position = as.numeric(df[2]), 
                                                             units = "hg38")
    
  }
}; rm(nonrsid.cases, df, n, signif.pval, chr_id, b.gwas, b.studies); gc()

# snps.mb <- snps.from.rsid(rsid = gwas_32424353_29059683[which(gwas_32424353_29059683$CHR_ID == "")]$SNPS,
#                           dbSNP = SNPlocs.Hsapiens.dbSNP155.GRCh38,
#                           search.genome = BSgenome.Hsapiens.UCSC.hg38)

## These snps are not mapped to hg38. Will remove them
gwas_32424353_29059683 <- gwas_32424353_29059683[-which(gwas_32424353_29059683$REGION == ""),] %>%
  distinct()

#save(gwas_32424353_29059683, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/gwas_32424353_29059683.rda")
```
...
snps.df <- gwas_finemap %>% dplyr::select(-REGION) %>%
  dplyr::rename("seqnames"="CHR_ID", "start"="CHR_POS", "SNP_id"="SNPS") %>% ## Rename columns
  mutate(seqnames = paste0("chr", seqnames)) %>% ## add chr before chromosome id
  distinct() ## Keep unique observations

#gene.tss <- geneAnnot.df %>% dplyr::select(seqnames, tss, x) %>% dplyr::rename("gene_name" = "x")
gene.tss <- geneAnnot.df %>% dplyr::select(seqnames, tss, gene_name, gene_type)

gwas.list <- as.list(as.data.frame(t(snps.df[, c("seqnames", "start", "SNP_id")])))
...
save(nearbygenes_10xgenomics, file = "/drive-pool/data/peter_data/genetic_programming/breast_nearbygenes_200kb_elmer_10xgenomic.rda")
...
promoter.granges <- promoters(genes.granges, upstream = 5000, downstream = 1000)
...
promoter.df <- cbind(as.data.frame(gwas) %>% .[hits@from,],
  ...
  mutate(promoter = 1)
save(promoter.df, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/promoter.df.rda")
```
...
gwas <- GRanges(seqnames = Rle(paste0("chr", gwas_finemap$CHR_ID)),
                range = IRanges(start = as.numeric(gwas_finemap$CHR_POS)),
                locus = gwas_finemap$SNPS)
...
coding.hits <- cbind(as.data.frame(gwas) %>% .[hits@from,],
  ...
  distinct() %>%
  mutate(coding = 1)
#save(coding.hits, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/coding.hits.rda") ## 23 hits in coding
...
celltalkdb1 <- celltalkdb %>% group_by(grp = paste(pmax(target_genesymbol, source_genesymbol), pmin(target_genesymbol, source_genesymbol), sep = "_")) #%>%
