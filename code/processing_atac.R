...
gwas <- GRanges(seqnames = Rle(paste0("chr", gwas_finemap$CHR_ID)),
                range = IRanges(start = as.numeric(gwas_finemap$CHR_POS) - 500,
                                end = as.numeric(gwas_finemap$CHR_POS) + 499),
                locus = gwas_finemap$SNPS,
                chr_pos = as.numeric(gwas_finemap$CHR_POS))
...
atac.hm <- as.data.frame(table(atac.marker$locus, atac.marker$celltype))
pheatmap(atac.hm)

#save(atac.marker, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/atac.marker.rda")        
...
## Linking atac peaks to snps
load("/drive-pool/data/peter_data/genetic_programming/code/brca/gwas_finemap.rda")
## Make gwas snps into granges
gwas <- GRanges(seqnames = Rle(paste0("chr", gwas_finemap$CHR_ID)),
                range = IRanges(start = as.numeric(gwas_finemap$CHR_POS) - 1000,
                                end = as.numeric(gwas_finemap$CHR_POS) + 1000),
                locus = gwas_finemap$SNPS,
                chr_pos = as.numeric(gwas_finemap$CHR_POS))
...
save(common.atac.hit, file = "/drive-pool/data/peter_data/genetic_programming/code/brca/common_atac_peak.rda")
...
dev.off()
png(filename = "~/atactest.png", height = 150, width = 150, unit = "mm", res = 300)
grid::grid.draw(p)
dev.off()
