...
control_group <- paste0("control", c(1:4))
...
p1 <- ggplot(data = combined.data, aes(x = n.clust, y = max.size, color = type)) +
  ...
  annotate("text", x = 12, y=37, label = paste0("KS = ", signif(ks.maxsize$p.value, digits = 4)), size = 3)
...
p2 <- ggplot(data = combined.data, aes(x = n.clust, fill = type)) +
  ...
  guides(fill = "none") +
  annotate("text", x = 12, y=0.15, label = paste0("KS = ", round(ks.nclust$p.value, digits = 3)), size = 3) +
...
set.seed(514)
plot(network, 
     vertex.color = cell.anno,
     vertex.label.color = "black",
     vertex.label.font = 2,
     vertex.label.cex = 1.5,
     vertex.size = 20,
     edge.width = 4)
text(c(0.9, -0.4, -1.2, 1.0, 0.15, -0.55), c(0.6, -1.1, 0.6, -0.75, -0.35, -0.30), c("1", "2", "3", "4", "5", "6"), cex = 2, family = 2)
legend("bottomleft", legend=levels(as.factor(V(network)$value))[1], col = legend.anno[1], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[2], col = legend.anno[2], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.05))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[3], col = legend.anno[3], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.1))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[4], col = legend.anno[4], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.15))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[5], col = legend.anno[5], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.20))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[6], col = legend.anno[6], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.25))
legend("bottomleft", legend=levels(as.factor(V(network)$value))[7], col = legend.anno[7], bty = "n", pch = 20, pt.cex = 3, cex = 1, 
       text.col= "black", horiz = TRUE, inset = c(0,0.3))
...
ig <- image_read("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/figures/consensus_interaction_network.png")
p3 <- image_ggplot(ig) 
...
dev.off()
combined.plot <- p2 + p1 + plot_spacer() + p4 + plot_spacer() + plot_layout(design = layout) + plot_annotation(tag_levels = "A") &
...
ggsave("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/figure/figure_6.png", width = 170, height = 170,
       units = "mm", dpi = 300)
...
dev.off()
combined.plot <- p3 + plot_spacer() + plot_layout(design = layout) &
  theme(legend.position = 'bottom',
        plot.tag = element_text(face = 'bold', size = 10),
        axis.text = element_text(size = 8),
        axis.title = element_text(size = 8),
...
ggsave("/drive-pool/data/peter_data/repo/genetic_algorithm_gwas_sc/figures/figure_6.png", width = 170, height = 170,
       units = "mm", dpi = 300)
