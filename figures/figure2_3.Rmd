...
p4 <- ggplot(data = of.plotdata, aes(x = gen, y = score, group = of)) +
  ...
  geom_label(aes(label = new.of), data = of.plotdata %>% filter(gen == 55, !new.of %in% c("Promoter", "Cancer Gene", "LPI", "Marker Gene")), nudge_x = 0.35, size = 2) +
  geom_label(aes(label = new.of), data = of.plotdata %>% filter(gen == 55, new.of %in% c("Promoter")), nudge_x = 0.35, nudge_y = 0.02, size = 2) +
  geom_label(aes(label = new.of), data = of.plotdata %>% filter(gen == 55, new.of %in% c("Cancer Gene")), nudge_x = 0.35, nudge_y = -0.01, size = 2) +
  geom_label(aes(label = new.of), data = of.plotdata %>% filter(gen == 55, new.of %in% c("LPI")), nudge_x = 0.35, nudge_y = -0.02, size = 2) +
  geom_label(aes(label = new.of), data = of.plotdata %>% filter(gen == 55, new.of %in% c("Marker Gene")), nudge_x = 0.35, nudge_y = 0.02, size = 2) +
  theme(panel.grid = element_blank(),
        axis.line = element_line(colour = "black"))
